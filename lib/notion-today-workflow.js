...
function getPropertyText(property) {
  ...
  /*
   * Check the actual populated value first instead of relying only
   * on property.type. This is more resilient to API schema changes.
   */
...
function groupWorkflowItems(items, today) {
  ...
  items.forEach((item) => {
     ...
     * Scheduled has the highest priority.
     */
