...
function parseSavingsBucket(
  ...
  /*
   * Transfers into sinking funds are
   * entered as negative transactions.
   * Normalize the rollup into a positive
   * amount saved.
   */
   ...
   * Zero = goal reached exactly
   * Negative = amount over goal
   */
  const remaining =
    goalAmount -
    currentBalance;
  ...
  /*
   * Calculate progress from the normalized
   * balance instead of the Notion formula.
   */
  const progress =
    goalAmount > 0
      ? (
...
async function getFinanceSinkingFunds() {
  const pages =
