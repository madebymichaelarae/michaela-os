...
import { getTodayWorkflow } from "../notion-today-workflow.js";
import { getApprovalSummary } from "../notion-approval-summary.js";
...
function findProperty(properties, expectedName) {
  ...
  const matchingKey = Object.keys(properties).find(
    (propertyName) =>
      normalize(propertyName) === normalizedExpected
  );
...
function getRequestedMonth(request) {
  ...
  const queryYear = Number(request.query?.year);
  const queryMonth = Number(request.query?.month);
...
function dateBelongsToMonth(
  ...
  const expectedMonth = String(month).padStart(
    2,
    "0"
  );

  const expectedPrefix = `${year}-${expectedMonth}`;
...
async function handleContentSummary(
  ...
  const entries = await queryContentEntries({
    sorts: [
      {
        property: PROPERTY_NAMES.sendDate,
        direction: "ascending"
      }
    ]
  });
  ...
  for (const entry of entries) {
    ...
    const clientName = getPropertyText(
      clientProperty
    );
    ...
    const isEmail = isExactValue(
      contentType,
      CONTENT_TYPES.email
    );

    const isText = isExactValue(
      contentType,
      CONTENT_TYPES.text
    );
    ...
    const deliverableSummary = isEmail
      ? clientSummary.emails
      : clientSummary.texts;

    deliverableSummary.owed += 1;

    if (isExactValue(status, SENT_STATUS)) {
...
async function handleTodayWorkflow(response) {
...
async function handleApprovalSummary(response) {
