...
function getDateKey(
  ...
  /*
   * Notion often returns date-only values,
   * such as "2026-07-29".
   *
   * Turning that directly into a JavaScript
   * Date can shift it to the previous day in
   * America/New_York.
   *
   * Date-only strings should therefore be
   * preserved exactly as Notion returned them.
   */
  if (
...
function buildStatsResponse(
  ...
  for (const session of sessions) {
    ...
    /*
     * Only count a date toward the
     * reading streak when the session
     * contains actual reading activity.
     */
    if (
      pagesRead > 0 ||
      minutes > 0
