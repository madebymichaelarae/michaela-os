...
import { getFinanceHeader } from "./finance/header.js";
import { getFinanceAccounts } from "./finance/accounts.js";
  ...
  if (
    ...
    simplified.rollup = {
      relationPropertyName:
        property.rollup
          .relation_property_name || null,

      rollupPropertyName:
        property.rollup
          .rollup_property_name || null,

      function:
        property.rollup.function || null
    };
  }
...
function simplifyDataSource(dataSource) {
...
export async function handleFinanceRequest(req) {
  const view = String(
    req.query?.view || "schema"
  )
    .trim()
    .toLowerCase();
  ...
  switch (view) {
    ...
    default:
      return {
        success: false,
        error: `Unknown finance view: ${view}`
      };
  }
}
