...
async function findPreviousEndPage(
  bookId
) {
  const dataSourceId =
    await getDataSourceId(
...
async function createReadingSession({
  ...
  /*
   * Entry Title exists in your Reading Log.
   * We detect the actual title property
   * automatically instead of relying on its name.
   */
  const titleEntry =
    Object.entries(
...
export default async function readingLogHandler(
  ...
  try {
    ...
    /*
     * Shortcut still sends this as
     * "currentPage". Conceptually it is
     * the END PAGE of the new session.
     */
    const endPage =
      Number(
        body.currentPage ??
    ...
    if (
      ...
      return response
        ...
        .json({
          ...
          error:
            "End page must be a valid number."
        });
    }
    ...
    /*
     * Preferred starting page:
     *
     * 1. Last Reading Log End Page
     * 2. Book Current Page
     * 3. Zero
     */
    const previousLogEndPage =
      await findPreviousEndPage(
        currentBook.id
      );
    ...
    const startPage =
      typeof previousLogEndPage ===
        "number"
        ? previousLogEndPage
        : (
            typeof bookCurrentPage ===
              "number"
              ? bookCurrentPage
              : 0
          );
    ...
    if (
      ...
      return response
        ...
        .json({
          ...
          error:
            `End page (${endPage}) cannot be before start page (${startPage}).`
        });
    }

    /*
     * We calculate this locally for the
     * response and habit threshold.
     *
     * Notion's Pages Read formula remains
     * the official stored calculation.
     */
    const pagesRead =
      Math.max(
        0,
    ...
    /*
     * Get the previous daily total before
     * creating the new page. This avoids
     * waiting for Notion's new page/formula
     * to become queryable immediately.
     */
    ...
    /*
     * Keep Books.Current Page synchronized
     * with the latest Reading Log.
     */
